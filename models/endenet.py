...
def net(ls, input, train):
    ######################
    # hl = [3,32,64,32,64,32,3] # CDED_L adam op
    net = tf.layers.conv2d(input, ls[1], [3, 3], strides=(2, 2), padding='SAME', activation=tf.nn.relu,
                           trainable=train)  # n_batchx192x192x32
    print("conv1 ", net)

    net = tf.layers.conv2d(net, ls[2], [3, 3], strides=(2, 2), padding='SAME', activation=tf.nn.relu,
